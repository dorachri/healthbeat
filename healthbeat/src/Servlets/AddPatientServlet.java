	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if (!usernames.isEmpty() && !amkas.isEmpty() && !emails.isEmpty()) {
				...
				request.getRequestDispatcher("add_patient.jsp").include(request, response);
			}
				...
				byte salt[] = Passwords.getNextSalt(); //create a random hash
				char[] passwordChar = password.toCharArray(); //covert password from String to CharArray
				byte[] newPassword = Passwords.hash(passwordChar, salt); //create the hashed password(plain text +salt)
				...
				registerUser.setBytes(3, newPassword);
